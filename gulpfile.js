...
gulp.task('clean', function () {
  return del(
    folder.build + 'styles.css',
    folder.build + 'main.js',
    folder.build + 'main.min.js'
  )
})
...
gulp.task(
  ...
  gulp.series('clean', 'images', function () {
    const postCssOpts = [assets({ loadPaths: ['assets/'] }), autoprefixer]
...
gulp.task(
  ...
  gulp.series(function (done) {
    gulp
      .src([folder.src + 'js/lib/*', folder.src + 'js/*'])
      .pipe(babel())
      .pipe(concat('main.js'))
      .pipe(gulp.dest(folder.build))
    done()
...
gulp.task(
  ...
  gulp.series('babel', function () {
    let jsbuild = gulp
      .src([folder.src + 'js/lib/*', folder.src + 'js/main.js']) // <- Multiple files need to go in an array
      .pipe(babel())
      .pipe(concat('main.min.js'))

    jsbuild = jsbuild.pipe(stripdebug()).pipe(terser())
